...
random_sentences = random.sample(sentences, 10000)
...
center_sentences = [
    ...
    #
    # 'Spatial Relation',
    # 'Action, State, Temporal Context',
    # 'Association, purpose',
    # 'Comparison'
]

cluster_num = len(center_sentences)
...
center_texts = tokenizer(center_sentences)
texts = texts.to(device)
center_texts = center_texts.to(device)

with torch.no_grad(), torch.cuda.amp.autocast():
    text_features = clip.encode_text(texts)
    center_text_features = clip.encode_text(center_texts)

# 将 PyTorch 张量转换为 NumPy 数组，方便 KMeans 使用
text_features_np = text_features.cpu().numpy()
center_text_features_np = center_text_features.cpu().numpy()

# Step 3: 使用 KMeans 对文本特征进行聚类
kmeans = KMeans(n_clusters=cluster_num, init=center_text_features_np, n_init=1)  # 例如选择 10 个聚类
...
#
#     # 找到这个点对应的文本句子
#     closest_sentence = random_sentences[np.where(kmeans.labels_ == i)[0][closest_idx]]
#
#     # 将该文本作为该聚类的标签
...
for i in range(cluster_num):  # 假设 10 个聚类
    ...
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], label=center_sentences[i], color=colors(i))
...
for idx, label in enumerate(kmeans.labels_):
    if center_sentences[label] not in cluster_texts:
        cluster_texts[center_sentences[label]] = []
    cluster_texts[center_sentences[label]].append(modified_random_sentences[idx])
