...
class DetrDatasetMapperUni:
    ...
    def __init__(self, cfg, is_train=True, test_categories=None):
        ...
        self.openclip_input = cfg.INPUT.OPENCLIP_INPUT
        # if self.openclip_input:
        #     _, train_preprocess, val_preprocess = open_clip.create_model_and_transforms(cfg.MODEL.LANGUAGE_BACKBONE.MODEL_TYPE, pretrained=cfg.MODEL.LANGUAGE_BACKBONE.PRETRAINED)
        #     self.tfm_gens = self.tfm_gens[:-1] + train_preprocess.transforms[:1]
        #     self.crop_gen = None
        ...
        if self.lang_guide_det:
            ...
            self.all_class_name = {}
            self.all_class_name["coco"] = [value for key,value in self.ind_to_class_dict["coco"].items()][1:]
            self.all_class_name["obj365v2"] = [value for key,value in self.ind_to_class_dict["obj365v2"].items()][1:]
            if cfg.MODEL.LANGUAGE_BACKBONE.NAME == 'openclip':
                self.tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE)
            elif cfg.MODEL.LANGUAGE_BACKBONE.NAME == 'bert':
                ...
                if use_roberta:
                    self.tokenizer = RobertaTokenizerFast.from_pretrained('projects/CLARE/roberta-base')
                else:
                    self.tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE) # align with GLIP# align with GLIP
    ...
    def transform_annos(self, annotations_ori, transforms, image_shape, dataset_dict):
        ...
        if self.lang_guide_det and task == "detection":
            ...
            all_class_name = self.all_class_name[dataset_dict["dataset_name"]]
            original_box_num = len(instances)
            if self.language_backbone == 'openclip':
                positive_map = torch.zeros((len(instances), len(all_class_name)), dtype=torch.bool)

                for i in range(len(instances)):
                    label_i = instances.gt_classes[i].item()
                    positive_map[i, label_i] = True
                instances.positive_map = positive_map
                expressions_new = all_class_name
            else:    
    ...
    def __call__(self, dataset_dict):
        ...
        if not self.is_train:
            ...
            if self.lang_guide_det and task == "detection":
                ...
                if self.language_backbone == 'openclip':
                    dataset_dict["expressions"] = self.all_class_name[dataset_dict["dataset_name"]]
                    # dataset_dict["positive_map_label_to_token"] = None
            return dataset_dict
    ...
    def __call__(self, target):
        ...
        if self.return_tokens and self.tokenizer is not None: # True
            tokenized = self.tokenizer(caption, return_tensors="pt",
                ...
                truncation=True)
            target["positive_map"] = create_positive_map(tokenized, target["tokens_positive"],self.max_query_len) # (N, 256) N is num of objects. value > 0 where positive class
...
def create_positive_map(tokenized, tokens_positive, max_query_len):
