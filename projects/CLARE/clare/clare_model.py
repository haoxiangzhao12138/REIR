...
from .models.deformable_detr.openclip_text_model import OpenclipTextEncoder
...
class CLARE(nn.Module):
    """
    CLARE: Contrastive Language-Instance Alignment with Relation Experts
    """
    ...
    def __init__(self, cfg):
        ...
        if 'bert' in cfg.MODEL.LANGUAGE_BACKBONE.NAME:
        ...
        elif 'openclip' in cfg.MODEL.LANGUAGE_BACKBONE.NAME:
            # openclip
            self.tokenizer = AutoTokenizer.from_pretrained(cfg.MODEL.LANGUAGE_BACKBONE.TOKENIZER_TYPE)
            if self.parallel_det:
                self.text_encoder = OpenclipTextEncoder(cfg)
            else:
                self.text_encoder = nn.Sequential(OrderedDict([("body", OpenclipTextEncoder(cfg))]))
        ...
        self.detr = model_class(
            ...
            cls_pool_type=cfg.MODEL.CLS_POOL_TYPE,
        ...
        if cfg.MODEL.USE_BATCH_LOSS:
            weight_dict.update({"loss_batch": cfg.MODEL.DDETRS.BATCH_LOSS_WEIGHT})
        
        if cfg.MODEL.USE_SIGLIP_LOSS:
            weight_dict.update({"loss_siglip": cfg.MODEL.DDETRS.SIGLIP_LOSS_WEIGHT})
        
        if cfg.MODEL.USE_CONTRASTIVE_FOCAL_LOSS:
            weight_dict.update({"loss_contrastive": cfg.MODEL.DDETRS.CONTRASTIVE_FOCAL_LOSS_WEIGHT})
        
        if cfg.MODEL.USE_CLIP_POOL_HEAD:
            weight_dict.update({"loss_text_image": cfg.MODEL.DDETRS.TEXT_IMAGE_LOSS_WEIGHT})
        ...
        # self.retrieval_grounding = cfg.MODEL.RETRIEVAL_GROUNDING
        self.use_bias = cfg.MODEL.USE_BIAS
        self.topk = cfg.MODEL.OBJECT_QUERY_TOPK
        self.score_thres = cfg.MODEL.OBJECT_QUERY_THRES
    ...
    def forward(self, batched_inputs, do_postprocess=True):
        ...
        if self.training:
            ...
            # if 
            if self.retrieval and batched_inputs[0]['annotations']: 
                results = self.inference_retrieval_grounding(box_cls, box_pred, mask_pred, images.image_sizes, positive_map_label_to_token, num_classes, score_thres=self.score_thres, task=task, iou_pred=iou_pred, output=output)
            elif self.retrieval:
                results = self.inference_retrieval(box_cls, box_pred, mask_pred, images.image_sizes, positive_map_label_to_token, num_classes, score_thres=self.score_thres, task=task, iou_pred=iou_pred, output=output)
                return results
            else:
                results = self.inference(box_cls, box_pred, mask_pred, images.image_sizes, positive_map_label_to_token, num_classes, score_thres=self.score_thres, task=task, iou_pred=iou_pred)
    ...
    def inference(self, box_cls, box_pred, mask_pred, image_sizes, positive_map_label_to_token, num_classes, score_thres=0.0, task=None, iou_pred=None):
        ...
        for i, (logits_per_image, box_pred_per_image, image_size, iou_per_image) in enumerate(zip(
            ...
            if self.ota:
                # NMS
                if self.language_backbone == "bert":
    ...
    def inference_retrieval_grounding(self, box_cls, box_pred, mask_pred, image_sizes, positive_map_label_to_token, num_classes, score_thres=0.0, task=None, iou_pred=None, output=None):
        """
        Arguments:
            box_cls (Tensor): tensor of shape (batch_size, num_queries, K).
                The tensor predicts the classification probability for each query.
        ...
        assert len(box_cls) == len(image_sizes)
        results = []
        vis_feat = output["vis_features"]
        lang_feat = output["lang_features"]
        if self.use_bias:
            bias = output["bias"]
            scale = output["scale"]
        for i, (logits_per_image, box_pred_per_image, image_size, iou_per_image) in enumerate(zip(
            ...
            keep_indices = ops.batched_nms(boxes_before_nms,nms_scores,idxs,0.7)  
            prob = prob[keep_indices]
            current_vis_feat = vis_feat[:, keep_indices, :]
            box_pred_per_image = box_pred_per_image[keep_indices]
            if mask_pred is not None:
                mask_pred_i = mask_pred[i][keep_indices]
            
            
            _, topk_indices = torch.topk(prob.view(-1), self.topk, dim=0)
            prob = prob[topk_indices]
            box_pred_per_image = box_pred_per_image[topk_indices]
            if self.mask_on:
                mask_pred_i = mask_pred_i[topk_indices]
            current_vis_feat = current_vis_feat[:, topk_indices, :]
            
            scores_per_image = prob
            ...
            result.scores = scores_per_image
            result.lang_features = lang_feat.repeat(box_pred_per_image.shape[0], 1, 1)
            result.vis_features = current_vis_feat.squeeze(0)
            if self.use_bias:
                result.bias = bias
                result.scale = scale
            results.append(result)
    ...
    def inference_retrieval(self, box_cls, box_pred, mask_pred, image_sizes, positive_map_label_to_token, num_classes, score_thres=0.0, task=None, iou_pred=None, output=None):
        assert len(box_cls) == len(image_sizes)
        results = []
        vis_feats = output["vis_features"]
        lang_feats = output["lang_features"]
        clip_image_features = output["clip_image_features"]
        clip_text_features = output["clip_text_features"]

        for i, (logits_per_image, box_pred_per_image, image_size, iou_per_image, vis_feat, lang_feat, clip_image_feature, clip_text_feature) in enumerate(zip(
            box_cls, box_pred, image_sizes, iou_pred, vis_feats, lang_feats, clip_image_features, clip_text_features
        )):
            max_num_inst = 100
            # NMS
            logits_per_image = convert_grounding_to_od_logits(logits_per_image.unsqueeze(0), num_classes, positive_map_label_to_token)
            ...
            keep_indices = ops.batched_nms(boxes_before_nms,nms_scores,idxs,0.7)  
            prob = prob[keep_indices]
            vis_feat = vis_feat[keep_indices]
            if iou_per_image is not None:
                iou_per_image = iou_per_image[keep_indices]
            ...
            if score_thres > 0.0:
                ...
                vis_feat = vis_feat[valid_mask]
                if iou_per_image is not None:
                    iou_per_image = iou_per_image[valid_mask].squeeze(1)


            result = Instances(image_size)
                
            result.lang_features = lang_feat
            result.vis_features = vis_feat.unsqueeze(0)
            result.clip_image_features = clip_image_feature.unsqueeze(0)
            result.clip_text_features = clip_text_feature.unsqueeze(0)
            if iou_per_image is not None:
                result.iou_thresh = iou_per_image.unsqueeze(0)
            results.append(result)
        return results
    
    def scale(self, bbox, scale_x, scale_y):
        """
    ...
    def forward_text(self, captions, device, task=None):
        if isinstance(captions[0], list) and len(set(tuple(caption) for caption in captions)) == 1:
            captions = captions[0]
