...
class BertEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        if self.bert_name == "bert-base-uncased":
            config = BertConfig.from_pretrained("projects/CLARE/%s"%self.bert_name)
            config.gradient_checkpointing = self.cfg.MODEL.LANGUAGE_BACKBONE.USE_CHECKPOINT
            self.model = BertModel.from_pretrained("projects/CLARE/%s" % self.bert_name, add_pooling_layer=False, config=config)
            ...
            self.language_dim = 768
        # elif self.bert_name == "DFN5B-CLIP-ViT-H-14-378":
