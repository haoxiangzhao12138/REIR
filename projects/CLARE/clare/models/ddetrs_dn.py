...
class DDETRSegmUniDN(nn.Module):
    def __init__(self, detr, rel_coord=True, ota=False, new_mask_head=False, use_raft=False, mask_out_stride=4, \
        ...
        if self.dynamic_label_enc:
            self.resizer = FeatureResizer(
                    input_feat_size=cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM,
    ...
    def coco_forward(self, samples, gt_targets, criterion, train=False, language_dict_features=None, task=None):
        ...
        features, pos, image_features = self.detr.backbone(samples)
        ...
        if self.dynamic_label_enc:
            ...
            elif self.language_backbone == "openclip":
                lang_feat_pool = language_dict_features["hidden"]
        ...
        for lvl in range(enc_lay_num):
            ...
            if task == "grounding":
                outputs_class, outputs_logits_batch = self.detr.class_embed[lvl](hs[lvl], lang_feat_pool)
            elif task == "detection":
                if self.language_backbone == "openclip":
                    class_features = language_dict_features["hidden"].unsqueeze(0).repeat(hs[lvl].size(0), 1, 1)
                else:
                    class_features = language_dict_features["hidden"]
                outputs_class, outputs_logits_batch = self.detr.class_embed[lvl](hs[lvl], class_features)
            else:
                raise ValueError("task must be detection or grounding")
            ...
            all_outputs_logits_batch.append(outputs_logits_batch)
            if self.use_iou_branch:
            ...
            # if task == "detection" and self.language_backbone == "openclip":
            #     num_class = gt_targets[0]["positive_map"].size(1)
            #     for i in range(len(gt_targets)):
            #         gt_targets[i]["positive_map"] = torch.eye(num_class, dtype=torch.bool)
        ...
        if self.use_clip_pool_head:
            zimg = F.normalize(image_features, dim=1)
            ztxt = F.normalize(lang_feat_pool.squeeze(1), dim=1)
            outputs['image_features'] = zimg
            outputs['text_features'] = ztxt
        
        outputs['pred_logits'] = outputs_class[-1]
        outputs['pred_boxes'] = outputs_coord[-1]
        outputs['pred_masks'] = outputs_mask[-1]
        outputs['logits_batch'] = all_outputs_logits_batch[-1]
        if self.use_iou_branch:
        ...
        if self.detr.aux_loss:
            ...
            for i in range(len(outputs['aux_outputs'])):
                outputs['aux_outputs'][i]["text_masks"] = text_masks
                outputs['aux_outputs'][i]["logits_batch"] = all_outputs_logits_batch[i]
            # for x in outputs['aux_outputs']:
            #     x["text_masks"] = text_masks
            #     x["logits_batch"] = all_outputs_logits_batch
        ...
        loss_dict = criterion(outputs, gt_targets, indices_list, dn_meta)
    ...
    def coco_inference(self, samples, gt_targets, criterion, train=False, language_dict_features=None, task=None):
        ...
        if getattr(self, "use_deformable_reid_head", False):
            hs, memory, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact, language_dict_features, src_info = \
                self.detr.transformer(srcs, masks, poses, query_embeds, mask_on=True, language_dict_features=language_dict_features, task=task,
                attn_masks=attn_mask, return_src_info=True)
        ...
        if task == "grounding" or task == "sot":
            if self.retrieval:
                outputs_class, vis_features, lang_features,bias, scale = self.detr.class_embed[lvl](hs[lvl], lang_feat_pool, return_feature=self.retrieval)
            else:
                outputs_class, outputs_logits_batch = self.detr.class_embed[lvl](hs[lvl], lang_feat_pool)
        elif task == "detection":
            if self.language_backbone == "openclip":
        ...
        outputs['pred_boxes'] = outputs_coord[-1]
        if self.retrieval:
            outputs['lang_features'] = lang_features
            outputs['vis_features'] = vis_features
            # outputs['clip_image_features'] = F.normalize(image_features, dim=1)
            # outputs['clip_text_features'] = F.normalize(lang_feat_pool.squeeze(1), dim=1)
            if self.use_bias:
                outputs['bias'] = bias
                outputs['scale'] = scale
