...
class ReirLocalVisualizer(Visualizer):
    ...
    def add_datasample(
            ...
            if 'gt_instances' in data_sample:
                if 'text' in data_sample:
                    gt_img_data = self._draw_instances(image,
                                                       ...
                                                       classes, palette, data_sample.text)
                else:
                    gt_img_data = self._draw_instances(image,
                                                   data_sample.gt_instances,
                                                   classes, palette)

            # if 'ref_gt_instances' in data_sample:
            #     gt_img_data = self._draw_instances(gt_img_data,
            #                                        data_sample.ref_gt_instances,
            #                                        classes, palette, data_sample.text)
        ...
        if draw_pred and data_sample is not None:
            ...
            # if 'ref_result' in data_sample:
            #     rec = InstanceData()
            #     rec.bboxes = data_sample.ref_result.unsqueeze(0)
            #     rec.labels = torch.zeros(1, dtype=torch.int64)
            #     pred_img_data = self._draw_instances(pred_img_data,
            #                                        rec,
            #                                        classes, palette, data_sample.text)
